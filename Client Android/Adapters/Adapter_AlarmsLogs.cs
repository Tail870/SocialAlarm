...
namespace Client_Android
{
    public class Adapter_AlarmsLogs : RecyclerView.Adapter
    {
        ...
        public override void OnBindViewHolder(RecyclerView.ViewHolder viewHolder, int position)
        {
            ...
            else { this.viewHolder.textViewIsAlarm.Text = Application.Context.Resources.GetString(Resource.String.isReminder); }
            this.viewHolder.textViewTime.Text = alarmsLogs[position].DateTime.ToLocalTime().DateTime.ToString("HH:mm\ndd/MM/yyyy");
            ...
            else { this.viewHolder.textViewUsername.Text = alarmsLogs[position].UserSlept; }
            if (alarmsLogs[position].DisplayedNameSlept != null && alarmsLogs[position].DisplayedNameSlept.Trim().Length > 0)
            { this.viewHolder.textViewUsernameWaker.Text = alarmsLogs[position].DisplayedNameWaker; }
            else { this.viewHolder.textViewUsernameWaker.Text = alarmsLogs[position].UserWaker; }
        }
